...
class Laberinto:
    def __init__(self, level):
        ...
        pygame.display.set_caption(f"Chasqui Laberinto - Nivel {level}")
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.USEREVENT:
                self.move_delay = 150  # Resetear velocidad
    ...
    def solve_maze_astar(self):
        ...
        return []  # No se encontro camino seguro

    def toggle_ai_solving(self):
        # Activar/desactivar la resolucion por IA
        ...
        if self.ai_solving and self.ai_algorithm:
            if self.ai_algorithm == 'DFS':
                ...
                self.ai_path = self.solve_maze_astar()  # Por defecto, usar A*
            
            self.solving_steps = len(self.ai_path) - 1
            if not self.ai_path:
                print(f"No se encontro un camino seguro. La IA ({self.ai_algorithm}) no puede resolver el laberinto de forma segura.")
                self.ai_solving = False
            else:
    ...
    def save_solution_image(self):
        ...
        pygame.image.save(maze_surface, f"solucion_laberinto_nivel{self.level}_{self.ai_algorithm}.png")
        ...
        pygame.image.save(minimap_surface, f"minimapa_solucion_nivel{self.level}_{self.ai_algorithm}.png")
    ...
    def draw(self):
        ...
        # Dibujar botones de IA
        self.ai_button_rect = pygame.Rect(900, 200, 160, 50)
        ...
        self.dfs_button_rect = pygame.Rect(900, 260, 160, 50)
        self.bfs_button_rect = pygame.Rect(900, 320, 160, 50)
        ...
        # Mostrar pasos de la solucion IA
        if self.ai_solving and self.solving_steps > 0:
            self.draw_text(f"Pasos: {self.solving_steps}", (900, 380), color=(255, 255, 0))
